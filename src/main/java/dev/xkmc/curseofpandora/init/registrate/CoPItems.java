...
public class CoPItems {
	...
	static {
		...
		// attributes
		{
			...
			CHARM_HEALTH = item("charm_of_health", p -> new AttributeItem(p,
					AttributeItem.add(Attributes.MAX_HEALTH, "charm_of_health", CoPConfig.SERVER.attr.charmOfHealth::get)))
					.tag(CoPTagGen.ATTR, dupe).register();

			CHARM_ARMOR = item("charm_of_armor", p -> new AttributeItem(p,
					AttributeItem.add(Attributes.ARMOR, "charm_of_armor", CoPConfig.SERVER.attr.charmOfArmor::get),
					AttributeItem.add(Attributes.ARMOR_TOUGHNESS, "charm_of_armor", CoPConfig.SERVER.attr.charmOfArmorToughness::get)
			)).tag(CoPTagGen.ATTR, dupe).register();

			CHARM_SPEED = item("charm_of_speed", p -> new AttributeItem(p,
					AttributeItem.multBase(Attributes.MOVEMENT_SPEED, "charm_of_speed", CoPConfig.SERVER.attr.charmOfSpeed::get)))
					.tag(CoPTagGen.ATTR, dupe).register();

			CHARM_DAMAGE = item("charm_of_damage", p -> new AttributeItem(p,
					AttributeItem.multBase(Attributes.ATTACK_DAMAGE, "charm_of_damage", CoPConfig.SERVER.attr.charmOfDamage::get)))
					.tag(CoPTagGen.ATTR, dupe).register();

			CHARM_CRIT = item("charm_of_critical", p -> new AttributeItem(p,
					AttributeItem.add(L2DamageTracker.CRIT_DMG, "charm_of_critical", CoPConfig.SERVER.attr.charmOfCritical::get)))
					.tag(CoPTagGen.ATTR, dupe).register();

			CHARM_BOW = item("charm_of_archery", p -> new AttributeItem(p,
					AttributeItem.add(L2DamageTracker.BOW_STRENGTH, "charm_of_archery", CoPConfig.SERVER.attr.charmOfArchery::get)))
					.tag(CoPTagGen.ATTR, dupe).register();

			CHARM_PROTECTION = item("charm_of_protection", p -> new AttributeItem(p,
					AttributeItem.multTotal(L2DamageTracker.REDUCTION, "charm_of_protection", () -> -CoPConfig.SERVER.attr.charmOfProtection.get())))
					.tag(CoPTagGen.ATTR, dupe).register();

			CHARM_MAGIC = item("charm_of_magic", p -> new AttributeItem(p,
					AttributeItem.add(L2DamageTracker.MAGIC_FACTOR, "charm_of_magic", CoPConfig.SERVER.attr.charmOfMagic::get)))
					.tag(CoPTagGen.ATTR, dupe).register();

			CHARM_EXPLOSION = item("charm_of_explosion", p -> new AttributeItem(p,
					AttributeItem.add(L2DamageTracker.EXPLOSION_FACTOR, "charm_of_explosion", CoPConfig.SERVER.attr.charmOfExplosion::get)))
					.tag(CoPTagGen.ATTR, dupe).register();

			CHARM_HEAVY = item("charm_of_heavy_weapon", p -> new AttributeItem(p,
					AttributeItem.multBase(Attributes.ATTACK_DAMAGE, "charm_of_heavy_weapon", CoPConfig.SERVER.attr.charmOfHeavyWeapon::get),
					AttributeItem.add(Attributes.ATTACK_SPEED, "charm_of_heavy_weapon", () -> -CoPConfig.SERVER.attr.charmOfHeavyWeaponSlow.get())
			)).tag(CoPTagGen.ATTR).register();

			CHARM_ACCURACY = item("charm_of_accuracy", p -> new AttributeItem(p,
					AttributeItem.add(L2DamageTracker.CRIT_RATE, "charm_of_accuracy", CoPConfig.SERVER.attr.charmOfAccuracy::get),
					AttributeItem.add(Attributes.ATTACK_SPEED, "charm_of_accuracy", () -> -CoPConfig.SERVER.attr.charmOfAccuracySlow.get())
			)).tag(CoPTagGen.ATTR).register();

			CHARM_LUCK = item("charm_of_luck", p -> new AttributeItem(p,
					AttributeItem.add(Attributes.LUCK, "charm_of_luck", CoPConfig.SERVER.attr.charmOfLuck::get)))
					.tag(CoPTagGen.ATTR).register();

			ORB_SOULGUARD = item("orb_of_soulguard", p -> new AttributeItem(p,
					AttributeItem.multTotal(Attributes.MOVEMENT_SPEED, "orb_of_soulguard", () -> -CoPConfig.SERVER.attr.orbOfSoulGuardSpeed.get()),
					AttributeItem.multTotal(L2DamageTracker.REDUCTION, "orb_of_soulguard", () -> -CoPConfig.SERVER.attr.orbOfSoulGuardReduction.get()),
					AttributeItem.add(CoPAttrs.SPELL, "orb_of_soulguard", () -> 1)))
					.tag(CoPTagGen.ATTR).register();

			ORB_EXECUTOR = item("orb_of_executor", p -> new AttributeItem(p,
					AttributeItem.multBase(Attributes.ATTACK_DAMAGE, "orb_of_executor", CoPConfig.SERVER.attr.orbOfExecutorAttack::get),
					AttributeItem.multTotal(Attributes.MAX_HEALTH, "orb_of_executor", () -> -CoPConfig.SERVER.attr.orbOfExecutorHealth.get())))
					.tag(CoPTagGen.ATTR).register();

			ORB_PROSECUTOR = item("orb_of_prosecutor", p -> new AttributeItem(p,
					AttributeItem.add(L2DamageTracker.CRIT_DMG, "orb_of_prosecutor", CoPConfig.SERVER.attr.orbOfProsecutorCritDmg::get),
					AttributeItem.add(L2DamageTracker.CRIT_RATE, "orb_of_prosecutor", () -> -CoPConfig.SERVER.attr.orbOfProsecutorCritRate.get())))
					.tag(CoPTagGen.ATTR).register();
		}
		...
		// enchs
		{
			...
			STABLE_BODY = descItem("orb_of_stability", "Orb of Stability", StabilityCharm::new,
					...
					.tag(slot, CoPTagGen.PANDORA_BASE).register();

			GOLDEN_HEART = item("golden_heart",
					p -> new PiglinShinnyCharm(p, LCEnchantments.SHINNY))
					.tag(slot, CoPTagGen.PANDORA_BASE).register();

			ENDER_CHARM = item("charm_of_calmness",
					p -> new EnderMaskCharm(p, LCEnchantments.ENDER_MASK))
					.lang("Charm of Calmness")
					.tag(slot, CoPTagGen.PANDORA_BASE).register();

			BLESS_SNOW_WALKER = item("bless_of_snow_walker",
					p -> new SnowWalkerCharm(p, LCEnchantments.SNOW_WALKER))
			...
			OWNER_PROTECTION = item("orb_of_master",
					p -> new EnchDescItem(p, LCEnchantments.IMM_MATES))
					.lang("Orb of Master")
					.tag(slot, CoPTagGen.PANDORA_BASE).register();

			ENVIRONMENTAL_REJECT = item("orb_of_nature",
					p -> new EnchDescItem(p, LCEnchantments.IMM_ENVIRONMENT))
					.lang("Orb of Nature")
					.tag(slot, CoPTagGen.PANDORA_BASE).register();

			EXPLOSION_REJECT = item("orb_of_explosion_rejection",
					p -> new EnchDescItem(p, LCEnchantments.IMM_EXPLOSION))
					.lang("Orb of Explosion Rejection")
					.tag(slot, CoPTagGen.PANDORA_BASE).register();

			PROJECTILE_REJECT = item("orb_of_projectile_rejection",
					p -> new EnchDescItem(p, LCEnchantments.IMM_PROJECTILE))
					.lang("Orb of Projectile Rejection")
					.tag(slot, CoPTagGen.PANDORA_BASE).register();

			MAGIC_REJECT = item("orb_of_magic_rejection",
					p -> new EnchDescItem(p, LCEnchantments.IMM_MAGIC))
	...
	public static void register() {
		...
		EntityFeature.SNOW_WALKER.add(new CurioFeaturePredicate(CoPItems.BLESS_SNOW_WALKER::get));
	}
