...
public class GLMDropItem extends Item {
	...
	public void appendHoverText(ItemStack stack, @Nullable Level level, List<Component> list, TooltipFlag flag) {
		...
		if (data != null) {
			...
			if (chance < 1) {
				list.add(CoPLangData.IDS.KILL_DROP_CHANCE.get(killer, target, (int) Math.round(data.chance * 100)).withStyle(ChatFormatting.GRAY));
			} else {
				list.add(CoPLangData.IDS.KILL_DROPS.get(killer, target).withStyle(ChatFormatting.GRAY));
			}
		}
	}
